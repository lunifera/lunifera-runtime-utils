...
public class RepoIndex implements ResourceIndexer {
     ...
     */
    public void index(Set<File> files, OutputStream out,
        ...
        try {
             ...
             * </pre>
             */
            indent = (prettySetting == null || (!Boolean
                    .parseBoolean(prettySetting) && compressedSetting != null)) ? Indent.NONE
                    : Indent.PRETTY;
            ...
            if (!compressed) {
                ...
                pw = new PrintWriter(new GZIPOutputStream(out, 8*1024){
                    {
            ...
            for (File file : filesToIndex) {
                try {
                    ...
                    log(LogService.LOG_WARNING, MessageFormat.format(
        ...
        for (File file : files) {
            if (!file.isDirectory()) {
                ...
                if (dirFiles.length > 0) {
                    Set<File> dirFilesSet = new LinkedHashSet<File>(
    ...
    private Tag generateResource(File file, Map<String, String> config)
        ...
        List<Capability> caps = new AddOnlyList<Capability>(
                new LinkedList<Capability>());
        List<Requirement> reqs = new AddOnlyList<Requirement>(
        ...
        try {
            ...
            if (config != null) {
                ...
                if (rootURLStr != null) {
                    ...
                    rootURL = new File(System.getProperty("user.dir")).toURI()
                            .toURL();

                String urlTemplate = config.get(ResourceIndexer.URL_TEMPLATE);
                bundleAnalyzer.setStateLocal(new GeneratorState(rootURL,
                        urlTemplate));
            } else {
                        ...
                        if (filter == null
                            ...
                            try {
                                ...
                                log(LogService.LOG_ERROR,
                                        MessageFormat
                                                .format("Error calling analyzer \"{0}\" on resource {1}.",
                                                        analyzer.getClass()
                                                                .getName(),
    ...
    private static void appendAttributeAndDirectiveTags(Tag parentTag,
        ...
        for (Entry<String, Object> attribEntry : attribs.entrySet()) {
            ...
            TypedAttribute typedAttrib = TypedAttribute.create(
                    attribEntry.getKey(), attribEntry.getValue());
        ...
        for (Entry<String, String> directiveEntry : directives.entrySet()) {
            ...
            directiveTag
                    .addAttribute(Schema.ATTR_NAME, directiveEntry.getKey());
    ...
    public IndexResult indexFile(File file) throws Exception {
        ...
        synchronized (analyzers) {
            for (Pair<ResourceAnalyzer, Filter> entry : analyzers) {
                ...
                if (filter == null
                        || filter.match(result.resource.getProperties())) {
                    analyzer.analyzeResource(result.resource,
                            result.capabilities, result.requirements);
                }
            }
        }
        return result;
